...
func (bcR *BlockchainReactor) ReceiveEnvelope(e p2p.Envelope) {
	...
	bcR.Logger.Error("Receive - blockchain/v1/reactor", "src", e.Src, "chID", e.ChannelID, "msg", e.Message)
...
func (bcR *BlockchainReactor) Receive(chID byte, peer p2p.Peer, msgBytes []byte) {
	bcR.Logger.Error(">>>>>>>>>>>>>>>>>>>>>>>> Reactor Receive - blockchain/v1/reactor <<<<<<<<<<<<<<<<<<<<<<<<<")
	fmt.Printf("peer.ID(): %v\n", peer.ID())
	fmt.Printf("peer.RemoteIP(): %v\n", peer.RemoteIP())
	fmt.Printf("peer.SocketAddr().IP: %v\n", peer.SocketAddr().IP)
	bcR.Logger.Error(">>>>>>>>>>>>>>>>>>>>>>> - blockchain/v1/reactor <<<<<<<<<<<<<<<<<<<<<<<<<")
	msg := &bcproto.Message{}
