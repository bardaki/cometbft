...
func (r *BlockchainReactor) ReceiveEnvelope(e p2p.Envelope) {
	...
	r.logger.Error("Receive - blockchain/v2/reactor", "src", e.Src.ID(), "chID", e.ChannelID, "msg", e.Message)
...
func (r *BlockchainReactor) Receive(chID byte, peer p2p.Peer, msgBytes []byte) {
	r.Logger.Error(">>>>>>>>>>>>>>>>>>>>>>>> Reactor Receive - blockchain/v2/reactor <<<<<<<<<<<<<<<<<<<<<<<<<")
	fmt.Printf("peer.ID(): %v\n", peer.ID())
	fmt.Printf("peer.RemoteIP(): %v\n", peer.RemoteIP())
	fmt.Printf("peer.SocketAddr().IP: %v\n", peer.SocketAddr().IP)
	r.Logger.Error(">>>>>>>>>>>>>>>>>>>>>>> - blockchain/v2/reactor <<<<<<<<<<<<<<<<<<<<<<<<<")
	msg := &bcproto.Message{}
